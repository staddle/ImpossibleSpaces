        ...
        private void setFollowingRooms(Node room, bool active)
        {
            foreach (Door door in currentRoom.doors)
            {
                if (door.nextNode == null)
                {
                    ...
                    door.nextNode = createRandomRoom(new Vector2(door.position.x, door.position.z), new Vector2(p2ToP1.y, -p2ToP1.x).normalized, currentRoom, door, get().roomGeneratorOptions);
                }
            }
