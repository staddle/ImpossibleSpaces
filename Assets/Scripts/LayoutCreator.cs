...
public class LayoutCreator : MonoBehaviour
{
    ...
    private void buildPlayArea(Vector2 playArea, float wallHeight)
    {
        ...
        wall1.transform.position = new Vector3(playArea.x / 2, wallHeight / 2, 0);
        wall1.transform.Rotate(Vector3.right, 90);

        wall2.transform.position = new Vector3(playArea.x / 2, wallHeight / 2, playArea.y);
        wall2.transform.Rotate(Vector3.left, 90);

        wall3.transform.position = new Vector3(0, wallHeight / 2, playArea.y / 2);
        wall3.transform.Rotate(Vector3.left, 90);
        wall3.transform.Rotate(Vector3.back, 90);

        wall4.transform.position = new Vector3(playArea.x, wallHeight / 2, playArea.y / 2);
    ...
    private GeneralLayoutRoom createRandomGeneralLayoutRoom(Vector2 startingPoint, Vector2 rhythmDirection, RoomGeneratorOptions options)
    {
        ...
        float depth = randomFloat(options.minimumGeneralLayoutWidth, options.maximumGeneralLayoutRoomSize);
        ...
        Vector2 rhythmDirectionInverse = new(sign * rhythmDirection.y, -sign * rhythmDirection.x);

        // Start from startingPoint and extrude in any direction perpendicular to rhythmDirection
        Vector2 point1 = startingPoint + rhythmDirectionInverse * width * distribution;
        ...
        Vector2 point2 = startingPoint + new Vector2(-rhythmDirectionInverse.x, -rhythmDirectionInverse.y) * (width - width * distribution);

        // Third and fourth points are just first two points but the depth of the room added (rectangular room)
        Vector2 point3 = point2 + rhythmDirection * depth;
        Vector2 point4 = point1 + rhythmDirection * depth;

        if (!playArea.isInside(point1))
        {
